...
from services.snapshot_service import SnapshotService, SNAPSHOTS_DIR
...
# Silence token-bar access logs
class TokenBarFilter(logging.Filter):
    def filter(self, record):
        return '/token-bar' not in record.getMessage()

# Apply to uvicorn.access logger
logging.getLogger("uvicorn.access").addFilter(TokenBarFilter())
...
async def handle_chat_send(
    ...
    # ✨ بررسی context قبل از ارسال
    token_info = chat_manager.get_session_token_count(session_id)
    if token_info["warning_level"] == "blocked":
        raise HTTPException(
            status_code=400,
            detail="Context is full (100%). Please create a snapshot to continue."
        )
    ...
    # Smart compression before sending
...
async def stop_message_generation(message_id: int, db: Session = Depends(get_db)):
    ...
    # ✨ همیشه وضعیت رو cancelled کن - حتی اگه cancel_event نباشه
    chat_svc = ChatService(db)
    msg = db.query(models.ChatMessage).filter(models.ChatMessage.id == message_id).first()
    if msg and msg.status == "pending":
        chat_svc.update_message_content(message_id, "⏹ Stopped by user.", status="cancelled")
        logger.info(f"[STOP-DB] message_id={message_id} marked as cancelled in DB")
...
@app.get("/api/session/{session_id}/token-status")
async def get_token_status(session_id: int, db: Session = Depends(get_db)):
    """Get current token usage for a session."""
    chat_manager = ChatService(db)
    return chat_manager.get_session_token_count(session_id)


@app.get("/api/session/{session_id}/token-bar")
async def get_token_bar(session_id: int, db: Session = Depends(get_db)):
    """Get visual token bar for header display."""
    chat_manager = ChatService(db)
    return {
        "bar": chat_manager.get_token_bar(session_id),
        "info": chat_manager.get_session_token_count(session_id)
    }
...
@app.post("/api/session/{session_id}/snapshot")
async def create_snapshot(session_id: int, db: Session = Depends(get_db)):
    """
    Create a snapshot of the current session.
    1. Sends hidden prompt to LLM
    2. Saves response to file
    3. Clears session messages
    4. Returns snapshot content
    """
    from openai import OpenAI
    import httpx

    logger.info(f"[SNAPSHOT] Starting snapshot for session {session_id}")
    ...
    history_text = ""
    for msg in messages:
        if msg.content and msg.status == "done":
    ...
    try:
        ...
        logger.info(f"[SNAPSHOT] Calling LLM for session {session_id}...")
        t0 = time.time()
        ...
        elapsed = time.time() - t0
        logger.info(f"[SNAPSHOT] LLM response in {elapsed:.1f}s")

        if not response or not response.choices:
            raise ValueError("Empty response from LLM")

        snapshot_content = response.choices[0].message.content.strip()

        if not snapshot_content:
            raise ValueError("Empty snapshot content")

    except Exception as e:
        error_msg = str(e)
        logger.error(f"[SNAPSHOT] LLM call failed: {error_msg}")
        
        # اگه پاسخ empty بود، پیام بهتری بده
        if "Empty" in error_msg or "None" in error_msg:
            raise HTTPException(
                status_code=500,
                detail="مدل پاسخ خالی برگردوند. احتمالاً Context پر شده یا مدل در دسترس نیست. لطفاً دوباره تلاش کنید."
            )
        raise HTTPException(status_code=500, detail=f"Snapshot generation failed: {error_msg}")
    finally:
        if http_client:
            try:
                http_client.close()
    ...
    # Save to file
    filepath = snapshot_service.save_snapshot_to_file(
    ...
    deleted_count = snapshot_service.clear_session_messages(session_id)

    logger.info(f"[SNAPSHOT] Complete! Deleted {deleted_count} messages, saved to {filepath}")

    return {
        "status": "success",
        "message": "Snapshot created and saved",
        "snapshot_content": snapshot_content,
        "filepath": filepath,
...
async def list_snapshots(session_id: int = None):
    """List all snapshots, optionally filtered by session_id."""
    snapshot_service = SnapshotService(None)
    snapshots = snapshot_service.get_all_snapshots(session_id)
...
async def download_snapshot(filename: str):
    ...
    try:
        content = snapshot_service.read_snapshot(filepath)
        return {
            "filename": filename,
            "content": content,
...
async def delete_snapshot(filename: str):
    ...
    if snapshot_service.delete_snapshot(filepath):
